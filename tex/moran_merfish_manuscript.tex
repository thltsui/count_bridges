...
\subsection{Reverse Generative Process via Backward Monte Carlo}

\paragraph{Adjoint transport and the importance function.}
The reverse-time generation problem is structurally identical to \emph{backward Monte Carlo} as used in nuclear engineering and neutron transport.
In that context, one avoids simulating all forward particle trajectories by instead running a fictitious \emph{adjoint} process backward from the detector, weighted by an \emph{importance function} $\Phi^*(X, t)$ that encodes how much a particle in state $X$ at time $t$ contributes to the detector response.
Here, the analogue of the detector response is the target data distribution $p_0$, and the importance function is the time-marginal density $p_t(X)$.

By Anderson's theorem~\citep{anderson1982reverse}, the rates of the exact time-reversed CTMC are:
\begin{equation}
  R_{X \to Y}(t) = Q_{Y \to X}(t)\;\frac{p_t(Y)}{p_t(X)},
  \label{eq:anderson}
\end{equation}
where $Q_{Y \to X}$ are the forward rates (Eq.~\ref{eq:generator}) and the ratio $p_t(Y)/p_t(X)$ is the importance ratio $\Phi^*(Y)/\Phi^*(X)$ in backward Monte Carlo language.
Running the CTMC with these modified rates samples from $p_0$ exactly---but only if $p_t$ is known.

\paragraph{The Independent Pseudo-Marginal (IPM) approximation.}
For the interacting $N$-particle system, $p_t(X)$ is intractable.
We approximate it via the \emph{Independent Pseudo-Marginal} factorisation, which treats each cell independently by setting $\kappa=0$ in the density but retaining spatial coupling in the rates:
\begin{equation}
  \Phi^*(X, t) \;\approx\; \prod_{i=1}^{N} P_{\text{mut}}\!\bigl(x^{(i)}_t \mid \hat{x}_0^{(i)}, t\bigr),
  \label{eq:ipm}
\end{equation}
where $\hat{x}_0^{(i)}$ is the denoiser's prediction of cell $i$'s clean profile.
This is the continuous-time analogue of the independent-particle approximation in reactor physics, where the joint adjoint flux is factored into single-particle adjoint fluxes.

\paragraph{Gaussian surrogate for the importance ratio.}
Computing the exact Skellam likelihood $P_{\text{mut}}(x_t \mid \hat{x}_0, t)$ requires Bessel function evaluations---numerically expensive and incompatible with MPS acceleration.
Under the CLT approximation (accurate for large integrated rate $\mu(t)$), the Skellam displacement $x_t - \hat{x}_0 \approx \mathcal{N}(0, 2\mu(t)I_G)$, giving the surrogate importance function:
\begin{equation}
  \log \Phi^*(x^{(i)}_t, t) \;\approx\; -\frac{\|x^{(i)}_t - \hat{x}_0^{(i)}\|^2}{4\,\mu(t)},
  \qquad \mu(t) = \gamma_{\text{mut}}\!\Bigl(\tfrac{1}{1-t}-1\Bigr).
  \label{eq:gaussian_ipm}
\end{equation}
This squared-distance computation is fully vectorisable on MPS/GPU with no special function calls.

\paragraph{Guided reverse mutation.}
For gene $g$ of cell $i$, the importance ratio for a birth event $x_g^{(i)} \to x_g^{(i)} + 1$ is:
\[
  \frac{\Phi^*(x^{(i)} + e_g)}{\Phi^*(x^{(i)})}
  \approx \exp\!\Bigl(\frac{-2\,\delta_g^{(i)} - 1}{4\mu(t)}\Bigr),
\]
where $\delta_g^{(i)} = x_g^{(i)} - \hat{x}_{0,g}^{(i)}$ is the gene residual.
The guided reverse birth and death rates are therefore:
\begin{align}
  r_{\text{birth}}^{(i,g)} &= \gamma_{\text{mut}}\lambda(t) \cdot
    \exp\!\Bigl(\omega\,\frac{-2\delta_g^{(i)} - 1}{4\mu(t)}\Bigr), \notag\\
  r_{\text{death}}^{(i,g)} &= \gamma_{\text{mut}}\lambda(t)\,x_g^{(i)} \cdot
    \exp\!\Bigl(\omega\,\frac{2\delta_g^{(i)} - 1}{4\mu(t)}\Bigr),
  \label{eq:guided_rates}
\end{align}
with guidance weight $\omega > 0$.
When the current count exceeds the prediction ($\delta_g^{(i)} > 0$), deaths are amplified and births suppressed; when it falls below, births are amplified.
This is the discrete-integer analogue of score-based guidance in continuous diffusion models~\citep{dhariwal2021diffusion}.

\paragraph{Adjoint resampling (backward coalescent).}
The same importance-weighting principle applies to the Moran resampling in reverse.
The adjoint resampling rate for cell $i$ to coalesce onto cell $j$'s full profile is:
\begin{equation}
  R_{\text{resamp}}(i \to j) = \frac{\kappa\,\lambda(t)}{N}\,K(p_i, p_j)\;
    \exp\!\Bigl(\omega\,\bigl[\log\Phi^*(x^{(j)}) - \log\Phi^*(x^{(i)})\bigr]\Bigr).
  \label{eq:adjoint_resamp}
\end{equation}
The exponential factor $e^{\omega\Delta\log\Phi^*}$ acts as an importance weight: cells with \emph{low} likelihood under the denoiser's prediction are preferentially overwritten by spatially proximate cells with \emph{high} likelihood.
In nuclear engineering terms, this is the adjoint-weighted coalescence: particle histories contributing more to the final response are amplified, while low-importance histories are collapsed.
The full $N \times N$ weight matrix $\mathcal{W}_{ij} = K(p_i, p_j)\cdot e^{\omega\Delta\log\Phi^*_{ij}}$ is computed in a single batched matrix operation; jump probabilities are $p_{\text{jump}}^{(i)} = \min(\frac{\kappa\lambda}{N}\sum_j\mathcal{W}_{ij}\Delta t,\;0.9)$, and the target cell is drawn from $\text{Categorical}(\mathcal{W}_i/\|\mathcal{W}_i\|_1)$.

Note that adjoint resampling performs \emph{whole-cell} coalescence ($x^{(i)} \leftarrow x^{(j)}$), in contrast to the gene-level copying of the forward process. This asymmetry is intentional: in generation, a cell rapidly adopts a well-fitting neighbour's full profile as a global correction, whereas the forward process uses gene-level diffusion to maintain transcriptional diversity.
...
We present the full pipeline in two self-contained algorithm blocks. Algorithm~\ref{alg:train} covers the EM training loop in detail; Algorithm~\ref{alg:gen} covers reverse-time generation at inference.

% -----------------------------------------------------------------------
\begin{algorithm}[H]
\caption{Moran Bridge: EM Training (from data to denoiser checkpoint)}
\label{alg:train}
\begin{algorithmic}[1]
\Require Dataset $\mathcal{D} = \{(X_0^{(s)}, \text{coords}^{(s)})\}$ of spots; hyperparameters $\gamma_{\text{mut}}, \kappa, h, \omega, K, T_{\text{EM}}, T_M$.
\Ensure Trained denoiser $f_\theta$.

\State \textbf{Precompute} reference counts $\mathcal{R} = \bigcup_s \text{cells}(s)$ for DP prior sampling.
\State Initialise denoiser $f_\theta$ (DeepSets, $d_h$=512, 5 enc/dec layers).
\State Initialise AdamW optimiser with cosine LR schedule.

\For{EM epoch $= 1, \ldots, T_{\text{EM}}$}
  \For{each minibatch $\mathcal{B} = \{(X_0^{(b)}, p^{(b)}, N^{(b)})\}_{b=1}^{B}$}

    \Statex \hfill\textit{--- E-step: infer latent cell profiles via Moran adjoint reverse ---}
    \State $f_\theta.\text{eval}()$
    \For{each spot $b \in \mathcal{B}$}
      \State \textbf{DP prior sample:} $x_{t_K}^{(b)} \leftarrow \text{DP-StickBreaking}(\theta, \mathcal{R}, N^{(b)})$
      \Comment{Eq.~\ref{eq:ipm}: importance fn initialised from truncated stick-breaking}
    \EndFor

    \For{$k = K, K{-}1, \ldots, 1$}
      \State $t_k \leftarrow k/K$;\quad $t_{k-1} \leftarrow (k-1)/K$;\quad $\Delta t \leftarrow 1/K$
      \State \textbf{Denoiser query:}
        $\hat{x}_0^{(b)} \leftarrow f_\theta\!\bigl(x_{t_k}^{(b)},\; t_k,\; \text{mask}^{(b)},\; p^{(b)}\bigr)$
        \hfill\textit{[B, N, G] in parallel}
      \State \textbf{IPF rescale:} $\hat{x}_0^{(b)} \leftarrow \hat{x}_0^{(b)} \cdot \frac{X_0^{(b)}}{\sum_i \hat{x}_{0,i}^{(b)} + \varepsilon}$
        \hfill\textit{enforce bulk sum constraint $\sum_i \hat{x}_0^{(i)} = X_0$}
      \If{$t_{k-1} < 0.01$} \Comment{Final step}
        \State \textbf{Randomised rounding:} $x_0^{(b)} \leftarrow \text{RandRound}(\hat{x}_0^{(b)},\, X_0^{(b)})$
        \hfill\textit{Poisson rounding preserving integer totals}
      \Else
        \State \textbf{Compute importance function} (Eq.~\ref{eq:gaussian_ipm}):
          $\log\Phi^*(x^{(b)}_i) = -\|x^{(b)}_{t_k,i} - \hat{x}_{0,i}^{(b)}\|^2 / (4\mu(t_k))$
        \State \textbf{Guided mutation} (Eq.~\ref{eq:guided_rates}): for each $(i,g)$:
        \Statex \qquad $r_{\text{birth}} = \gamma_{\text{mut}}\lambda(t_k)\exp(\omega\,(-2\delta_g^{(i)}-1)/4\mu)$;\;
                  $r_{\text{death}} = \gamma_{\text{mut}}\lambda(t_k) x_g^{(i)}\exp(\omega\,(2\delta_g^{(i)}-1)/4\mu)$
        \State \textbf{Adjoint resampling} (Eq.~\ref{eq:adjoint_resamp}): compute $\mathcal{W}_{ij} = K(p_i,p_j)\exp(\omega\,\Delta\log\Phi^*_{ij})$
        \Statex \qquad $p_{\text{jump}}^{(i)} = \min\!\bigl(\tfrac{\kappa\lambda}{N}\textstyle\sum_j \mathcal{W}_{ij}\,\Delta t,\; 0.9\bigr)$;
                  draw $m_i \sim \text{Bernoulli}(p_{\text{jump}}^{(i)})$;
                  if $m_i{=}1$: $x^{(b)}_i \leftarrow x^{(b)}_{s_i}$, $s_i \sim \text{Cat}(\mathcal{W}_i/\|\mathcal{W}_i\|_1)$
        \State $x_{t_{k-1}}^{(b)} \leftarrow x_{t_k}^{(b)}$ after mutation + resampling
      \EndIf
    \EndFor

    \Statex \hfill\textit{--- M-step: train denoiser on inferred profiles ---}
    \State $f_\theta.\text{train}()$
    \For{$m = 1, \ldots, T_M$}
      \State Sample $t \sim \text{Uniform}(0.05, 0.95)$ independently per spot
      \State \textbf{Forward Moran} (Eq.~\ref{eq:generator}): for each spot $b$ with $n = N^{(b)}$ cells:
      \Statex \qquad \textit{Mutation:} $\text{births} \sim \text{Poi}(\gamma_{\text{mut}}\lambda(t)\Delta\tau)$;\;
               $\text{deaths} \sim \text{Bin}(x_g, 1{-}e^{-\gamma_{\text{mut}}\lambda(t)\Delta\tau})$
      \Statex \qquad \textit{Gene-level resampling:} $m_g^{(i)} \sim \text{Ber}(p_{\text{resamp}}^{(i)})$;\;
               $s_g^{(i)} \sim \text{Cat}(K_i/\|K_i\|_1)$;\;
               $x_g^{(i)} \leftarrow x_g^{(s_g^{(i)})}$ if $m_g^{(i)}{=}1$
      \State \textbf{Compute loss:}
        $\mathcal{L} = \mathcal{L}_{\text{Chamfer}}^{L^2}(\hat{x}_0, x_0) + 0.1\,\mathcal{L}_{\text{agg}} + 5.0\,\mathcal{L}_{\text{zero}}$
      \State \textbf{Backprop + clip gradients} ($\|\nabla\|\leq 1$); AdamW step; cosine LR decay
    \EndFor
  \EndFor
  \If{epoch $\bmod 5 = 0$} Save checkpoint \EndIf
\EndFor
\end{algorithmic}
\end{algorithm}
...
% -----------------------------------------------------------------------
\begin{algorithm}[H]
\caption{Moran Bridge: Reverse Generation (checkpoint $\to$ per-cell profiles)}
\label{alg:gen}
\begin{algorithmic}[1]
\Require Trained $f_\theta$; observation $X_0 \in \mathbb{Z}_{\geq 0}^G$ (bulk spot); cell coords $p^{(1)},\ldots,p^{(N)}$; steps $K$; guidance weight $\omega$.
\Ensure Integer per-cell expression matrix $\hat{X} \in \mathbb{Z}_{\geq 0}^{N \times G}$.

\State \textbf{Initialise from DP prior:} $x_{t_K} \leftarrow \text{DP-StickBreaking}(\theta, \mathcal{R}, N)$
\hfill\textit{few atoms, heavy clustering}
\State Precompute spatial kernel $K_{ij} = \exp(-\|p_i - p_j\|^2 / 2h^2)$, $K_{ii} = 0$

\For{$k = K, K{-}1, \ldots, 1$}
  \State $t_k \leftarrow k/K$;\quad $\mu(t_k) \leftarrow \gamma_{\text{mut}}(1/(1{-}t_k) - 1)$;\quad $\Delta t \leftarrow 1/K$

  \Statex \hfill\textit{--- Denoiser query + IPF projection ---}
  \State $\hat{x}_0 \leftarrow f_\theta(x_{t_k}, t_k, \text{mask}, p)$
  \hfill\textit{DeepSets forward pass, $[N,G]$}
  \State $\hat{x}_0 \leftarrow \hat{x}_0 \cdot X_0 / (\sum_i \hat{x}_{0,i} + \varepsilon)$
  \hfill\textit{IPF rescale: enforce bulk constraint}

  \If{$k = 1$} \Comment{Final step: discretise}
    \State $\hat{X} \leftarrow \text{RandRound}(\hat{x}_0, X_0)$
    \hfill\textit{Poisson rounding; $\sum_i \hat{X}_i = X_0$ a.s.}
    \State \Return $\hat{X}$
  \EndIf

  \Statex \hfill\textit{--- Compute importance function ---}
  \State $\log\Phi^*_i \leftarrow -\|x_{t_k,i} - \hat{x}_{0,i}\|^2 / (4\mu(t_k))$
  \hfill\textit{Gaussian IPM surrogate, Eq.~\ref{eq:gaussian_ipm}}

  \Statex \hfill\textit{--- Guided reverse mutation (per gene, vectorised) ---}
  \State $\delta_g^{(i)} \leftarrow x_{t_k,g}^{(i)} - \hat{x}_{0,g}^{(i)}$
  \State $r_{\text{birth}}^{(i,g)} \leftarrow \gamma_{\text{mut}}\lambda(t_k) \exp\!\bigl(\omega(-2\delta_g^{(i)}{-}1)/4\mu\bigr)$
  \State $r_{\text{death}}^{(i,g)} \leftarrow \gamma_{\text{mut}}\lambda(t_k) x_g^{(i)} \exp\!\bigl(\omega(2\delta_g^{(i)}{-}1)/4\mu\bigr)$
  \State $x_{t_k,g}^{(i)} \mathrel{+}= \mathbf{1}[\text{Uniform}(0,1) < r_{\text{birth}}\Delta t]$
  \hfill\textit{birth events}
  \State $x_{t_k,g}^{(i)} \mathrel{-}= \mathbf{1}[x_g^{(i)}>0 \;\wedge\; \text{Uniform}(0,1) < r_{\text{death}}\Delta t]$
  \hfill\textit{death events}

  \Statex \hfill\textit{--- Adjoint resampling (backward coalescent, per cell) ---}
  \State Recompute $\log\Phi^*_i$ after mutation
  \State $\mathcal{W}_{ij} \leftarrow K_{ij} \cdot \exp\!\bigl(\omega(\log\Phi^*_j - \log\Phi^*_i)\bigr)$
  \hfill\textit{$[N,N]$ importance-weighted kernel}
  \State $p_{\text{jump}}^{(i)} \leftarrow \min\!\bigl(\tfrac{\kappa\lambda(t_k)}{N}\sum_j \mathcal{W}_{ij}\Delta t,\; 0.9\bigr)$
  \State Draw $m_i \sim \text{Bernoulli}(p_{\text{jump}}^{(i)})$ for all $i$
  \State For cells $i$ with $m_i = 1$: draw $s_i \sim \text{Categorical}(\mathcal{W}_i / \|\mathcal{W}_i\|_1)$;\; $x_{t_k}^{(i)} \leftarrow x_{t_k}^{(s_i)}$
  \hfill\textit{whole-cell coalescence}

  \State Clip: $x_{t_k} \leftarrow \text{clip}(x_{t_k}, 0, 500)$
\EndFor
\end{algorithmic}
\end{algorithm}
