...
To diffuse point coordinates over this integer grid, the original framework constructs a purely independent, non-interacting random walk modeled by independent Poisson birth-death processes with cumulative intensities governed by \(\Lambda_{\pm}(t)\). 

In the \textbf{Forward Process}, counts diffuse to an unconditional target. Because independent Poisson increments mathematically generate coordinate bounding variations, the transition kernel directly results in a symmetric \textbf{Skellam distribution} at \(t=1\), mapping coordinates to an unstructured stationary distribution.
In the \textbf{Backward Process}, the reverse generative step inherently evaluates the unobserved "slack" metrics (e.g. forward transitions where a birth strictly cancels a death symmetrically) resolved elegantly using a \textbf{Bessel analytical slack posterior}. Using a trained continuous energy-score model to estimate the terminal \(\hat{X}_0\), the discrete inference path navigates utilizing an exact Binomial-Hypergeometric generative reverse kernel to sequentially jump coordinates backward over time toward the target moons distribution.
